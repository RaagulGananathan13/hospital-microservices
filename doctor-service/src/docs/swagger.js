...
const getSwaggerSpec = (req) => {
  ...
  const servers = [{ url: `${directProto}://${directHost}`, description: 'Doctor service direct URL' }];

  if (forwardedHost && forwardedPrefix) {
    const proxyProto = forwardedProto || directProto;
    servers.unshift({
      url: `${proxyProto}://${forwardedHost}${forwardedPrefix}`,
      description: 'Doctor service via API gateway'
    });
  }
  ...
  return swaggerJsdoc({
    definition: {
      ...
      info: {
        ...
        version: '1.0.0',
        description: 'Doctor microservice documentation for Hospital Management System'
      },
      servers
    },
    apis: ['./src/routes/doctorRoutes.js']
  });
};
